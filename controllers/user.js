...
export const getUser = (req, res, next) => {
    ...
    req.user.isAdmin === true && Object.keys(req.query).length > 0
        ...
        ? false
        : req.user.isAdmin === false && Object.keys(req.query)[0] == '_id'
        ? (data = req.query)
...
export const changePassword = (req, res, next) => {
    ...
    if (newPass.length < 8) {
        return next(
            new ErrorResponse('password length must be more than 8', 411),
        );
    }
    ...
    User.findById(req.user._id)
        ...
        .then((nil) => {
            ...
            doc.save().then(res.status(200).json({ success: true }));
        })
        .catch((err) => {
            return next(new ErrorResponse(err.message));
        });
